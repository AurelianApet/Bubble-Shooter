...
window.onload = function() {
    ...
    function addBubbles() {
        ...
        for (var i=0; i<level.columns; i++) {
            ...
            if(rowoffset == 0 && i == 10) level.tiles[i][0].type = -1
            else level.tiles[i][0].type = getExistingColor();
        }
        // if(rowoffset == 0) {
        //   level.tiles[i][10].type = -1; 
        // }
    }
    ...
    function getTileCoordinate(column, row) {
        ...
        if ((row + rowoffset) % 2) {
          if(column == 10) tilex -= level.tilewidth/2;
          else tilex += level.tilewidth/2;
        }
